...
class BatchRunner:
    ...
    def run_analysis(
        ...
        """Run games and collect full replay data for post-hoc analysis.

        Same batched inference loop as run_eval, but records every decision
        into the ReplayCollector with logits, value estimates, and game state.

        Returns (wins, losses, total_steps).
        """
        ...
        while games_completed < num_games:
            ...
            for i in range(self.num_concurrent):
                if games[i] is None or not games[i].is_game_over:
                    continue
                winner = games[i].get_winner() or ""
                if winner == self.training_agent_name:
                    wins += 1
                else:
                    losses += 1
                total_steps += step_counts[i]
                collector.finish_game(i, winner, games[i].stats.total_turns, opponent_type)
                games_completed += 1
                if games_started < num_games:
                    games[i] = self._start_eval_game()
                    step_counts[i] = 0
                    ...
                    self._advance_non_ppo(games[i])
                    if games[i].is_game_over:
                        w = games[i].get_winner() or ""
                        if w == self.training_agent_name:
                            wins += 1
                        else:
                            losses += 1
                        total_steps += step_counts[i]
                        collector.finish_game(i, w, games[i].stats.total_turns, opponent_type)
                        games_completed += 1
                        games[i] = None
                else:
                    games[i] = None
            ...
            masked_logits = logits_batch.masked_fill(masks_batch == 0, float('-inf'))
            ...
            values_cpu = values_batch.cpu()
            masks_cpu = masks_batch.cpu()
            act_indices_cpu = act_indices.tolist()
