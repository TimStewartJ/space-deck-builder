...
def test_train_cli_token_features_are_experimental_opt_in():
    default_args = _parse_train_args([])
    token_args = _parse_train_args(["--token-features"])

    assert default_args.token_features is False
    assert token_args.token_features is True
    assert token_args.self_play is False
    assert token_args.opponents == "random,heuristic,simple"
